...
1. Изменен формат дерева элементов активного окна для MCP Runner и внешних ИИ-агентов: визуальные элементы теперь описываются стабильным полем `ID`, дочерние элементы хранятся в `Items`, а сравнение снимков окна сопоставляет элементы по `ID`.
2. Расширено получение состояния активного окна: для групп и страниц может возвращаться `CurrentVisibleTab`, колонки таблиц определяются в том числе внутри вложенных групп, а обработка сворачиваемых элементов стала устойчивее к ошибкам платформы.
3. Оптимизировано получение главного меню для ИИ-агентов: структура меню может сохраняться и повторно использоваться через тестируемый клиент.
4. Обновлены инструкции агентского окружения: сценарии ориентированы на использование `ID`, добавлены Claude-навыки для выбора значений, поиска и сортировки в списках.
5. Обновлены поставочные артефакты Тестера, агентской базы и внешней компоненты.

# 1.3.5.18

1. Расширен API сценариев: добавлен метод `ОткрытьЗначениеВПолеВвода`, который открывает значение поля ввода и устанавливает открывшуюся форму текущей.
2. Уточнено поведение подключения при обработке запросов MCP: автоматическая очистка ошибок отключается, если параметр `ClearErrors` не задан явно.
3. Обновлены инструкции агентского окружения: ядро Runner учитывает методы `ОткрытьЗначениеВПолеВвода` и `Фокус`, добавлен навык сортировки динамических списков и табличных частей.
4. Обновлены поставочные артефакты Тестера и агентской базы.
